...
export function RoiCalculator({ highRiskCount }: { highRiskCount: number }) {
  ...
  const calc = useMemo(() => {
    ...
    return { subscriptionLabel, exposureCr, ratio };
  ...
  return (
    <div className="glass glass-hover rounded-xl p-5">
      ...
      <div className="mb-2.5 rounded-xl border border-low/30 bg-low-bg p-3.5 px-4.5 text-[14px] font-semibold text-low-dark">
        {highRiskCount > 0
          ? `A single average breach with shadow-IT involvement (₹${calc.exposureCr.toFixed(2)} Cr) costs roughly ${calc.ratio.toFixed(0)}x this org's illustrative annual NETRA subscription — and this view currently shows ${highRiskCount} high-risk tool(s) that could be exactly that entry point.`
          : `A single average India data breach (₹${calc.exposureCr.toFixed(2)} Cr) costs roughly ${calc.ratio.toFixed(0)}x this org's illustrative annual NETRA subscription.`}
      </div>
